...
const AccountUser = () => {
    ...
    const leida = async (valor) => {
        await axios.put(`http://localhost:4000/api/check/${valor}`)
            .then(response => {
                //  console.log(response.data.response.check)
                let temporal = []
                respuesta.map((item) => {
                    if (response.data.response.check._id !== item._id && !response.data.response.check.check) {
                        return (
                            temporal.push(item)
                        )
                    }
                            ...
                            <div className="card-header background-card">
                                <ul className="nav nav-tabs card-header-tabs">
                                    <li className="nav-item">
                                        <a className="nav-link custom-nav-link custom-link active background-card-click" aria-current="true" href="#" onClick={() => actNoti("favorite")}>
                                            Favorites
                                        </a>
                                    </li>
                                    ...
                                    <li className="nav-item d-flex">
                                        <div>
                                            <a className="nav-link custom-nav-link background-card-click" href="#" onClick={() => actNoti("notifaciones")}>
                                        ...
                                        return (
                                            <div className="card">
                                                ...
                                                <LinkRouter to={`/equipment/${item.equipment}`}  className="myButton">
                                                    ...
                                                    <button onClick={() => leida(item._id)} className="myButton">
                                                        <CheckIcon />
                                                    </button>
                                                </div>
                                            </div>
                                            ...
                                            return (
                                                <Card sx={{ width: 340, margin: "20px", boxShadow: "1px 0px 5px 3px rgba(0,0,0,0.1)" }}>
                                                    <CardHeader
                                                        ...
                                                        avatar={
                                                            <FavoriteIcon
                                                                className={user && item.likes.includes(user.datosUser.id) ?
                                                                    "colorLike" : ""}
                                                    ...
                                                    <Box sx={{ display: "flex", justifyContent: "center", paddingBottom: 2 }}>
                                                        ...
                                                        <LinkRouter to={`/equipment/${item._id}`} className="myButton">
                                                            Read More
                                                        </LinkRouter>
